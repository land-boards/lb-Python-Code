...
* Can select multiple files one at a time
...
class ControlClass:
	...
	def theExecutive(self):
		...
		self.mapKickInList(theInList)
		self.countKickBoards(theInList)
		self.writeKickUSPSAddrBook(outCSVFile, theInList)
		self.writeKickPayPalAddrBook(outCSVFilePayPal, theInList)
	...
	def mapKickInList(self, theInList):
...
#		outStr += str(boardsCount)
	...
	def writeKickUSPSAddrBook(self, outFilePtr, theList):
	...
	def writeKickPayPalAddrBook(self, outFilePtr, theList):
		"""Write out the USPS Address book values.
		The output file is a CSV that can be read by the USPS Address Book Import.
