...
def getOff(locPair,pointsOnPath):
	#print("Looking for",locPair,"in",pointsOnPath)
	for currPoint in range(len(pointsOnPath)):
		#print("Checking",pointsOnPath[currPoint])
		if pointsOnPath[currPoint] == locPair:
			return currPoint
	assert False,"getOff error"
...
print("Length of colors on path",len(colorsOnPath))
print("colorsOnPath",colorsOnPath)
maxX = 0
minX = 0
maxY = 0
minY = 0
...
for yVal in range(maxY,minY-1,-1):
	for xVal in range(minX+1,maxX-1):
		#print ("xVal,yVal",xVal,yVal)
		locPair = [xVal,yVal]
		if locPair in pointsOnPath:
			off = getOff(locPair,pointsOnPath)
			#print("off =",off,"color =",colorsOnPath[off])
			if colorsOnPath[off] == 0:
				print(" ",end="")
			elif colorsOnPath[off] == 1:
				print("*",end='')
			else:
