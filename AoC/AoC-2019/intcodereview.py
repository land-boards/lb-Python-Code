...
# Note: looking into logger library, e.g. `import logging`
...
class CPU:
	...
	def __init__(self):
        ...
        # Note: default parameters w/ passing rather than hardcoding
	def loadIntCodeProgramToMemory(self, progName = 'input.txt'):
		""" 
		"""
		...
		with open(progName, 'r') as filehandle:  
                        # list(map(int, filehandle.readline().split(',')))
			self.programMemory = [int(charz) for charz in filehandle.readline().split(',')] + 10000*[0]
                # instead of
	...
	def setProgState(self,state):
        ...
        # This could also be a multiply
        # could also be n.zfill(5)
        # My recommendation is to pull it out of the class
	def padInstructionTo5Chars(self, instruction):
		...
		"""
                # condition can go right in the assert, then you don't need double negatives
                assert len(instruction) <= 5,"padInstructionTo5Chars: Something went wrong with an opcode (more than 5 chars"
	...
	def evaluatePairOfOpcodes(self, currentOp):
		...
		debug_BranchEval = False
                # Read up on tuples
                # if something's going to be resized, tuples are cheaper, quicker and immutable...
		if debug_BranchEval:
