...
def pushNodeAtPointToUnsolvedList(listOffset,parentNodeNumber):
	...
	"""
	debug_pushNodeAtPointToUnsolvedList = True
	global unsolvedNodesList
	global myList
	global nodeNumber
...
def findMaxSisterOff(nodeToFind):
	"""findMaxSisterOff
	"""
	print 'findMaxSisterOff: nodeToFindParent',nodeToFind
	maxOff = 0
...
def getSisterOffset(unsolvedNodesIndex):
	...
	if debug_getSisterOffset:
		print 'getSisterOffset: nodeElement',unsolvedNodesList[unsolvedNodesIndex],
	if debug_getSisterOffset:
...
def sisterSolution(unsolvedNodesIndex):
	...
	if debug_sisterSolution:
		print 'sisterSolution: node ', unsolvedNodesList[unsolvedNodesIndex][nodeNum]
	if unsolvedNodesList[unsolvedNodesIndex][nodeNum] == 0:
		if debug_sisterSolution:
			print 'sisterSolution: skip node',unsolvedNodesList[unsolvedNodesIndex][nodeNum]
		return False
	elif unsolvedNodesList[unsolvedNodesIndex] == -1:
		if debug_sisterSolution:
			print 'sisterSolution: Node is not yet solved below this node'
		return False
	elif isChildInEitherList(getSisterOffset(unsolvedNodesIndex)) == True:
		if debug_sisterSolution:
			print 'sisterSolution: Node is already in one of the lists'
		return False
	if debug_sisterSolution:
